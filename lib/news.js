...
const CONCURRENCY = 6; // lanzar todas las fuentes a la vez satura la conexion y hace fallar el resto
let cache = { at: 0, payload: null };

/** Ejecuta `items` con `fn`, como mucho `limit` a la vez. */
...
async function fetchHackerNews() {
  ...
  await mapWithConcurrency(HN_KEYWORDS, CONCURRENCY, async (kw) => {
      const url =
...
export async function getNews({ force = false } = {}) {
  ...
  const [feedResults, hnResult] = await Promise.all([
    mapWithConcurrency(active, CONCURRENCY, fetchFeed),
    fetchHackerNews(),
  ]);
  const results = [...feedResults, hnResult];
